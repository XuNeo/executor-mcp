...
All tools return plain text responses. Parameters shown below.
...
**Returns:** Plain text with `process_id`, `pid`, and `log` path (one per line)
...
**Returns:** New output text if `wait_time > 0`, otherwise "ok"
...
**Returns:** Buffered output text (last 1000 lines kept in memory)
...
**Returns:** One line per process: `{id} [{status}] pid={pid} {command}`
...
**Returns:** Plain text: pid, status, cmd, buffer size, log path, recent output
